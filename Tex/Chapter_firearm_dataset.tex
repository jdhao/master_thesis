...
\begin{figure}[h]
	...
	\bicaption{UKB 数据库中一些示例图片}{Some of the example images in UKB dataset}
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.8\linewidth]{chapter_firearm_facebook-gun-sell.jpg}
	\bicaption[Facebook 上一个枪支交易群]{Facebook 上一个枪支交易群\footnotemark}{A gun sell group in Facebook}
...
\begin{figure}[!t]
	...
	\bicaption{在线图像标注系统的前端评分界面}{The frontend scoring interface of the online labelling system}
...
\begin{figure}[t]
	...
	\bicaption[枪支数据库打分数据的偏差统计信息]{枪支数据库打分数据的偏差统计信息。\textbf{左}：总体的偏差信息；\textbf{右}: 个人的偏差信息}{The scoring deviation statistics of the Firearm dataset. \textbf{Left}: overall deviation; \textbf{Right}: personal deviation}
...
\begin{table}[h]
	\centering
	\bicaption{每个标注者有最大的个人偏差的类别数目}{The number of classes in which each labeller has the largest personal deviation}
...
\begin{figure}[t!]
	...
	\bicaption{枪支图像数据库各个类别图片数量}{The image number of different classes in the Firearm14k dataset}
...
\begin{figure}[t!]
	...
	\bicaption{Firearm14k 与其他数据库图片尺寸比值的概率分布}{The probability distribution of image size ratios in Firearm14k and other datasets}
...
\begin{figure}[!t]
	...
	\bicaption{来自 Firearm14k 数据库的一些示例图片}{Some of the example images from the Firearm14k dataset}
	\label{fig:dataset_sample_images}
\end{figure}


在图~\ref{fig:dataset_sample_images} 中，我们展示了一些随机选取自枪支数据库的图片，从图中可以看出，这些枪支图片中枪支的尺寸，拍摄的视角等变化千差万别，这给枪支图片的识别带来了很大的困难，也是我们试图解决的问题。
...
\begin{table}[h]
	\centering
	\bicaption[Firearm14k 数据库详细统计信息]{Firearm14k 数据库详细统计信息}{Detailed statistics of the Firearm14k dataset}
