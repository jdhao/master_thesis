...
\title[基于卷积神经网络的图像检索方法研究]{基于卷积神经网络的图像检索方法研究}
\author{郝杰东} % 作者姓名
...
%-> English titlepage
\englishtitle{Research on Approaches for Image Retrieval\\ Based on Convolutional Neural Networks}
...
图像检索在计算视觉领域一个非常重要而且经典的研究方向，同时，相关的技术在工业界也有非常广泛的应用。尽管该问题已经被研究多年，但却仍然面临很多的挑战，例如图像中物体的尺寸，姿态以及图像光照的变化都给图像检索算法的性能带来严重的干扰。深度卷积神经网络作为一种很好的图像特征表达模型，在图像分类，物体检测和图像语义分割等领域都取得了巨大的成功。在本论文中，我们研究了基于深度卷积神经网络的图像检索方法，论文的工作以及主要的贡献如下：

1. 提出了一种多尺度全卷积的图像实例检索方法。
...
\keywords{深度卷积神经网络，图像实例检索，多尺度特征表达，全卷积网络，双阈值对比损失函数，枪支检索}
...
\englishkeywords{deep convolutional neural network, visual instance retrieval, multi-scale feature representation, fully-convolutional network, double margin contrastive loss, firearm retrieval}
